...
let diagnostic_of_report =
  ...
  let source (report : Ocaml_loc.report) =
    ...
    | Ocaml_loc.Warning | Ocaml_loc.Unknown -> None
  in
  fun (error : Ocaml_loc.report) ->
    ...
    let message =
      Format.asprintf "@[%a@]" Ocaml_loc.print_main error |> String.trim
    in
...
class merlin_server =
  object (self)
    ...
    method private publish_diagnostics ~notify_back content =
      ...
      let diagnostics =
        try
          Merlin_jsoo.dispatch source query |> List.map ~f:diagnostic_of_report
        with _ -> []
      in
          ...
          | Some doc ->
              ...
              Lwt.return
                (try
                   ...
                   | Some result ->
                       let params =
                         List.map result.parameters
                           ~f:(fun (p : Query_protocol.signature_help_param) ->
                             ParameterInformation.create
                               ~label:(`Offset (p.label_start, p.label_end))
                               ())
                       in
                       let sig_info =
                         SignatureInformation.create ~label:result.label
                           ~parameters:params
                           ~activeParameter:(Some result.active_param) ()
                       in
                       SignatureHelp.create ~signatures:[ sig_info ]
      ...
      if n.method_ = "merlin/addCmis" then begin
        ...
        | Some params -> (
            ...
            (try
               ...
               let cmis =
                 List.map statics ~f:(fun j ->
                     ( j |> member "name" |> to_string,
                       j |> member "content" |> to_string ))
               in
            ...
            try
              ...
              List.iter dynamics ~f:(fun j ->
                  ...
                  let toplevel_modules =
                    j |> member "toplevelModules" |> to_list
