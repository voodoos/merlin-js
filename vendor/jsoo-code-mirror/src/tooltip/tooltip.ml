...
module Tooltip_view = struct
  ...
  let create ~dom ?get_coords ?overlap ?mount ?update ?positioned () =
    let get_coords =
      Option.map
        (fun get_coords ->
    ...
    Jv.set_if_some o "mount" @@ Option.map (fun mount ->
      Jv.repr (fun view -> mount (Editor.View.of_jv view))) mount;
    Jv.set_if_some o "update" @@ Option.map (fun update ->
      Jv.repr (fun view_up-> update (Editor.View.Update.of_jv view_up))) update;
...
module Tooltip = struct
  ...
  let create ~pos ?end_ ~create ?above ?strict_side ?arrow () =
    ...
    Jv.set o "create" @@ Jv.repr
      (fun view -> create (Editor.View.of_jv view) |> Tooltip_view.to_jv);
...
let hover_tooltip ?(config = Jv.null) (source : (view:Editor.View.t -> pos:int -> side: int -> Tooltip.t option Fut.t) ) =
  let source =
    Jv.repr @@ fun view pos side ->
      let fut = source ~view:(Editor.View.of_jv view) ~pos:(Jv.to_int pos)
        ~side:(Jv.to_int side)
      in
  ...
  in
  Jv.call tooltip "hoverTooltip" [|source|]
  |> Extension.of_jv
