...
module Tooltip_view : sig
  ...
  (** Creates a TooltipView:
      ...
      @param get_coords
        This method can be provided to make the tooltip view itself responsible
        for finding its screen position.
      @param overlap
        By default, tooltips are moved when they overlap with other tooltips.
        Set this to true to disable that behavior for this tooltip.
      @param mount
...
(** Creates a Tooltip:
    ...
    @param strict_side
      ...
      on that side to show the tooltip inside the viewport.
    @param arrow
      When set to true, show a triangle connecting the tooltip element to
      position pos.

    {{:https://codemirror.net/6/docs/ref/#tooltip.Tooltip} See the reference for
...
(** Options for hover tooltips:
    ...
    @param hover_on_change
      When enabled (this defaults to false), close the tooltip whenever the
      document changes.
    @param hover_time
...
(** Enable a hover tooltip, which shows up when the pointer hovers over ranges
    ...
    text. It should, if there is a tooltip associated with position pos return
    the tooltip description (either directly or in a promise). The side argument
